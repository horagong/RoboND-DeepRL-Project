...
#define LEARNING_RATE 0.005f //0.0f
#define REPLAY_MEMORY 10000
#define BATCH_SIZE 256 //8
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
    ...
    #else
		if (collisionWithProp || collisionWithGripper)
		{
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
    ...
    bool checkGroundContact = (gripBBox.min.z <= groundContact) || (gripBBox.max.z <= groundContact);
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			const float distGoal = BoxDistance(gripBBox, propBBox); //0; // compute the reward from distance to the goal
			...
			if( episodeFrames > 1 )
			{
				...
				rewardHistory = avgGoalDelta > 0 ? REWARD_WIN : (REWARD_LOSS * 20 * distGoal);
				newReward     = true;
